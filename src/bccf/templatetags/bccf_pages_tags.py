...
def page_menu(context, token):
    ...
    for part in parts:
        ...
        if isinstance(part, basestring):
            template_name = part
        elif isinstance(part, BCCFChildPage):
            parent_page = part
    ...
    if "menu_pages" not in context:
        ...
        rel = [m.__name__.lower() for m in BCCFChildPage.get_content_models()]
        published = BCCFChildPage.objects.published(for_user=user).select_related(*rel)
        ...
        if "page" not in context:
            ...
            except BCCFChildPage.DoesNotExist:
...
def models_for_pages(*args):
    ...
    return BCCFPageAdmin.get_content_models()
